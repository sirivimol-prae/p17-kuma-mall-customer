...
const CoinSidebar = () => {
  ...
  const getCoinIdFromTitle = (title: string) => {
    const coinAmount = title.match(/\d+/);
    if (coinAmount) {
      return `${coinAmount[0]}coin`;
    }
    return '1000coin';
  };
    ...
    <div className="pl-0">
      ...
      <div className="space-y-3 flex flex-col items-start">
        {displayedCoins.map((coin) => {
          const coinId = getCoinIdFromTitle(coin.title);
          
          return (
            <div key={coin.id} className="w-[235px] flex flex-col">
              <Link href={`/account/mycoin/${coinId}`} className="w-full">
